...
async function generateProgramAI(data, calculatedBodyFat) {
    ...
    2. קיבוע הגדרת המנות (Hardcoded Definitions) - חובה על המודל לייצר את אובייקט ה-portion_definitions תמיד עם הערכים המדויקים הבאים עבור מנה אחת:
       ...
       - חלבון (protein): 25g חלבון מלא, 0g פחמימה -> 125 קלוריות למנה. בסס את כל חישובי החלבון אך ורק על היעד של 25 גרם חלבון = 1 מנה.
    ...
    6. קיבוץ פירות (Fruit Grouping): חל איסור מוחלט לייצר פריט נפרד לכל פרי בבנק הפחמימות. במקום זאת, המודל חייב לייצר בדיוק 3 שורות (אובייקטים) עבור פירות, אחת לכל קבוצה, בפורמט הבא:
       - שם הפריט: 'פירות קבוצה א (בננה, תמרים, ענבים)' | כמות: '75 גרם'
       - שם הפריט: 'פירות קבוצה ב (תפוח, אגס, אפרסק, מנגו, אוכמניות)' | כמות: '125 גרם' (ובמקרה של אוכמניות 100 גרם).
       - שם הפריט: 'פירות קבוצה ג (תותים, אבטיח, מלון)' | כמות: '200 גרם'
    7. אופטימיזציה לזמן ריצה (Strict Token & Output Limits): עליך להפיק JSON תמציתי וממוקד בלבד כדי לחסוך זמן עיבוד:
       - בנק החלבונים יכיל מקסימום 11-12 פריטים מובילים (כולל אפשרויות משולבות וטבעוניות).
       - בנק הפחמימות יכיל מקסימום 9-10 פריטים (הכוללים את 3 שורות הפירות המקובצות + פחמימות מבושלות מובילות כמו אורז ופסטה).
       - בנק השומנים יכיל מקסימום 6 פריטים.
    8. חוק משקל מוכן לאכילה (Cooked Weight Only): המודל יחשב ויציג את כל משקלי המזון בגרמים בתפריטים ובבנק התחליפים אך ורק לפי משקל המזון לאחר בישול / אפייה / צלייה (מוכן לאכילה בצלחת). חל איסור מוחלט להציג ערכים או משקלים של מזון גולמי (נא/יבש).
       - חלבונים מהחי מבושלים (עוף, בקר, דגים): מנת חלבון אחת (15g חלבון מלא) תוגדר כ-50 גרם לאחר בישול.
       - פחמימות מבושלות (אורז, פסטה, בורגול, כוסמת, קינואה): מנת פחמימה אחת (15g פחמימה) תוגדר כ-50 עד 60 גרם מבושל (כ-3 כפות).
       - תפוחי אדמה ובטטה: מנת פחמימה אחת תוגדר כ-70 עד 80 גרם מבושל/אפוי.
    9. סינון איכות חלבון ואכיפת קטניות (High-Quality Protein Only):
       - אכיפה נוקשה על קטניות: חל איסור מוחלט להציג עדשים, שעועית או חומוס בבנק החלבונים. הם יופיעו אך ורק בבנק הפחמימות.
       - פריטי חובה (Must-Include Items): על המודל לכלול תמיד ובכל תפריט את הפריטים הבאים בבנק החלבונים, בניסוח המדויק הבא:
         * '100 גרם סלמון מבושל (סוגר בדיוק 1 מנת חלבון מלא ומקזז 3 מנות שומן מהסך היומי)'
         * '1 ביצה שלמה L (תורמת 7 גרם חלבון השווים לכ-0.3 מנת חלבון, ומקזזת 1 מנת שומן)'
         * '1 חלבון ביצה - הלבן בלבד (תורם 4 גרם חלבון השווים לכ-0.15 מנת חלבון, ללא שומן. טיפ: שילוב של 1 ביצה + 4-5 חלבונים ישלים למנה אחת)'
         * '80 גרם חזה עוף / הודו מבושל (סוגר 1 מנת חלבון מלא)'
         * '230 גרם קוטג' 5% (סוגר בדיוק 1 מנת חלבון מלא ומקזז 2 מנות שומן מהסך היומי)'
         * '85 גרם גבינה צהובה 9% - כ-3.5 פרוסות (סוגר בדיוק 1 מנת חלבון מלא ומקזז 1.5 מנות שומן)'
         * '190 גרם גבינה בולגרית או צפתית 5% (סוגר בדיוק 1 מנת חלבון מלא ומקזז 1.5 מנות שומן)'
         * 'גביע גדול של מעדן/יוגורט חלבון 0% המכיל 25 גרם חלבון (סוגר בדיוק 1 מנת חלבון מלא)'
         * 'כף מדידה אחת של אבקת חלבון WHEY - כ-33 גרם (סוגר בדיוק 1 מנת חלבון מלא)'
       - את שאר רשימת החלבונים (עד 11-12 פריטים סך הכל) על המודל להשלים ממקורות של חלבון מלא (בקר רזה, טופו, דג לבן).
    10. חוק העלות הכפולה (Composite & Separated Portions): קזז מאכלים מורכבים במקביל לפי המפתח הבא:
       - ביצה שלמה L: עולה 0.25 מנת חלבון + 1 מנת שומן.
       - חלבון ביצה (הלבן L): עולה כ-0.18 מנת חלבון (0 שומן). (1 ביצה שלמה + 4 חלבונים = 1.0 חלבון מלא ו-1.0 שומן).
       - סלמון מבושל (100 גרם): עולה בדיוק 1 מנת חלבון מלא + 3 מנות שומן.
       - בקר טחון 15% שומן מבושל (100 גרם): עולה 1 מנת חלבון מלא + 3.5 מנות שומן.
       - אדממה (100 גרם מבושל): עולה 1 מנת פחמימה + 0.5 מנת חלבון.
       - חמאת בוטנים / טחינה / אגוזים: עולים אך ורק בבנק השומן.
    11. הנגשה ושקיפות בממשק: 
       - חובה עליך להציג את 'תג המחיר' המלא של המאכלים המשולבים בבנק התחליפים בסוגריים (לדוגמה: 'חביתה מ-1 ביצה שלמה ו-2 חלבונים (תחליף זה סוגר בדיוק 1 מנת חלבון מלא ו-1 מנת שומן)').
       - עליך להוסיף הערה קבועה בראש התפריט המילולי: 'שים לב: כל משקלי המזון בגרמים המוצגים בתוכנית זו נשקלים לאחר בישול ומוכנים לאכילה'.
    ...
    5. חלוקת מאקרו-נוטריאנטים (Macro Split): תפריט לנשים יכיל יחס שומן גבוה יותר (30%-35% מהסך הקלורי) על חשבון הפחמימות. אצל גברים ניתן להסתמך יותר על פחמימות. יעד החלבון נשאר 1.8g-2.2g לכל ק"ג משקל גוף לשני המינים.
    6. יעדי אחוז שומן (Body Fat Targets): אם אתה מציב יעדי אחוז שומן, חטוב לגברים הוא 10%-15%, חטוב לנשים הוא 20%-24%. אין להציב יעד מתחת ל-18% לאישה אלא אם היא מתחרה מקצועית.
...
app.post('/api/regenerate-nutrition', authenticateToken, async (req, res) => {
    try {
        ...
        5. קיבוץ פירות (Fruit Grouping): חל איסור מוחלט לייצר פריט נפרד לכל פרי בבנק הפחמימות. במקום זאת, המודל חייב לייצר בדיוק 3 שורות (אובייקטים) עבור פירות, אחת לכל קבוצה, בפורמט הבא:
           - שם הפריט: 'פירות קבוצה א (בננה, תמרים, ענבים)' | כמות: '75 גרם'
           - שם הפריט: 'פירות קבוצה ב (תפוח, אגס, אפרסק, מנגו, אוכמניות)' | כמות: '125 גרם' (ובמקרה של אוכמניות 100 גרם).
           - שם הפריט: 'פירות קבוצה ג (תותים, אבטיח, מלון)' | כמות: '200 גרם'
        6. אופטימיזציה לזמן ריצה (Strict Token & Output Limits): עליך להפיק JSON תמציתי וממוקד בלבד כדי לחסוך זמן עיבוד:
           - בנק החלבונים יכיל מקסימום 11-12 פריטים מובילים (כולל אפשרויות משולבות וטבעוניות).
           - בנק הפחמימות יכיל מקסימום 9-10 פריטים (הכוללים את 3 שורות הפירות המקובצות + פחמימות מבושלות מובילות כמו אורז ופסטה).
           - בנק השומנים יכיל מקסימום 6 פריטים.
        ...
        8. סינון איכות חלבון ואכיפת קטניות (High-Quality Protein Only):
           - אכיפה נוקשה על קטניות: חל איסור מוחלט להציג עדשים, שעועית או חומוס בבנק החלבונים. הם יופיעו אך ורק בבנק הפחמימות.
            - פריטי חובה (Must-Include Items): על המודל לכלול תמיד ובכל תפריט את הפריטים הבאים בבנק החלבונים, בניסוח המדויק הבא:
              * '100 גרם סלמון מבושל (סוגר בדיוק 1 מנת חלבון מלא ומקזז 3 מנות שומן מהסך היומי)'
              * '1 ביצה שלמה L (תורמת 7 גרם חלבון השווים לכ-0.3 מנת חלבון, ומקזזת 1 מנת שומן)'
              ...
              * 'גביע גדול של מעדן/יוגורט חלבון 0% המכיל 25 גרם חלבון (סוגר בדיוק 1 מנת חלבון מלא)'
              * 'כף מדידה אחת של אבקת חלבון WHEY - כ-33 גרם (סוגר בדיוק 1 מנת חלבון מלא)'
           - את שאר רשימת החלבונים (עד 11-12 פריטים סך הכל) על המודל להשלים ממקורות של חלבון מלא (חזה עוף, בקר רזה, טופו, יוגורט יווני, דג לבן).
        9. חוק העלות הכפולה (Composite Portions): קזז מאכלים מורכבים במקביל לפי המפתח הבא:
           - ביצה שלמה L: עולה 0.25 מנת חלבון + 1 מנת שומן.
           - חלבון ביצה L: עולה כ-0.18 מנת חלבון (0 שומן).
           - סלמון מבושל (100 גרם): 1 חלבון מלא + 3 שומן.
           - בקר טחון 15% (100 גרם): 1 חלבון מלא + 3.5 שומן.
