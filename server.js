...
async function generateProgramAI(data, calculatedBodyFat) {
    ...
    7. אופטימיזציה לזמן ריצה (Strict Token & Output Limits): עליך להפיק JSON תמציתי וממוקד בלבד כדי לחסוך זמן עיבוד:
       - בנק החלבונים יכיל מקסימום 7-8 פריטים מובילים (כולל אפשרויות משולבות וטבעוניות).
    ...
    9. סינון איכות חלבון ואכיפת קטניות (High-Quality Protein Only):
       ...
       - פריטי חובה (Must-Include Items): על המודל לכלול תמיד ובכל תפריט את שלושת הפריטים הבאים בבנק החלבונים, בניסוח המדויק הבא:
       ...
       - את שאר רשימת החלבונים (עד 8 פריטים סך הכל) על המודל להשלים ממקורות של חלבון מלא (חזה עוף, בקר רזה, טופו, יוגורט יווני, דג לבן).
