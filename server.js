...
async function generateProgramAI(data, calculatedBodyFat) {
    ...
    9. סינון איכות חלבון ואכיפת קטניות (High-Quality Protein Only):
       ...
       - פריטי חובה (Must-Include Items): על המודל לכלול תמיד ובכל תפריט את שלושת הפריטים הבאים בבנק החלבונים, בניסוח המדויק הבא:
         * '100 גרם סלמון מבושל (סוגר 2 מנות חלבון ומקזז 2.5 מנות שומן)'
         * '1 ביצה שלמה L (סוגרת 0.5 מנת חלבון ומקזזת 1 מנת שומן)'
         * '1 חלבון ביצה - הלבן בלבד (סוגר 0.25 מנת חלבון, ללא השפעה על השומן)'
    ...
    10. חוק העלות הכפולה (Composite & Separated Portions): קזז מאכלים מורכבים במקביל לפי המפתח הבא:
       - ביצה שלמה L: עולה 0.5 מנת חלבון + 1 מנת שומן.
       - חלבון ביצה (הלבן L): עולה 0.25 מנת חלבון (0 שומן). (1 ביצה שלמה + 2 חלבונים = 1.0 חלבון ו-1.0 שומן).
       - סלמון מבושל (100 גרם): עולה 2 מנות חלבון + 2.5 מנות שומן.
       - בקר טחון 15% שומן מבושל (100 גרם): עולה 2 מנות חלבון + 3.5 מנות שומן.
